<section>
    ...
    <div class="flex">
        <svg  class="mt-2 mr-5" fill="#000000" height="180" width="180" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
	 viewBox="0 0 402.161 402.161" xml:space="preserve">
        ...
        <div class="mt-2 pt-4">
            ...
            <h1 class="font-medium text-2xl">{{ Auth::user()->name }}</h1>
            <small class="font-thin">
                e-mail
            </small>
            <h1 class="font-medium text-2xl">{{ Auth::user()->email }}</h1>
            <div class="flex">
                <div class="mr-3">
                    <small class="font-medium">
                        posts
                    </small>
                    <h1 class="font-medium text-2xl text-center">{{ count(auth()->user()->posts) }}</h1>
                </div>
                <div class="mr-3">
                    <small class="font-medium">
                        likes
                    </small>
                    <h1 class="font-medium text-2xl text-center">{{ count(auth()->user()->likes) }}</h1>
                </div>
                <div>
                    <small class="font-medium">
                        comments
                    </small>
                    <h1 class="font-thin text-2xl text-center">{{ auth()->user()->comments()->count() }}</h1>
                </div>
            </div>
        </div>
    </div>
